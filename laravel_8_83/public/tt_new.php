<?php
   ini_set('memory_limit', '-1');
	...
	$res = $conn->query("SELECT * FROM fetchReviews WHERE status = 0 and user_id in (3208,9440,9494,9228,10099,11475,11648) and user_id = 11648 limit 0, 1");
	if($res->num_rows > 0){
	...
	function fetchReviewsAndStoreInDB($asin, $user_id, $totalpage, $currentpage, $oldReviewsCnt, $processedPage) {
		...
		if($oldReviewsCnt < 1){  //fetching reviews for the first time for this product
			...
			if($i > $reviewPagesCount || $i > 25){ // if reviews are more than 25*20 //tushar //remove if reviews more than 25*20 are needed
...
//Below code needs changes.  -> TODO: redirect to the last page left
		...
		for(; ($i <= $reviewPagesCount || $i > 25); $i++){//TODO : 
