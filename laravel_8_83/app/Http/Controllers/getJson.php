...
if($dataArr['price'] == ''){
	...
	if(isset($price_str[1])){
		...
		preg_match('/<div\s*id="availability"\s*class="a-section\s*a-spacing-none|base">\s*<span\s*class="a-size-medium\s*a-color-success">(.*?)<\/span>\s*<\/div>/ms',$data,$in_stock_str);
		if($in_stock_str){
			$quantity = 1;
		...
		//	if($in_stock_str && trim($in_stock_str[1]) == 'Currently unavailable.'){
			if($in_stock_str && strpos($in_stock_str[1], 'Currently unavailable.') !== false){
				$quantity = 0;
				$dataArr['quantity'] = $quantity;
			}else{
