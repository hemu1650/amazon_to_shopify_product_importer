...
require 'awsApi.php';
//Associate Tag - betrononline-21
//AWS Access ID - AKIAIW44P57INUANBP4Q
//Secret key - 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
...
class AmzConfigController extends Controller
{
    ...
    public function store(AmzConfigRequest $request)
    {
		...
		if($amzKey){
			...
			echo $associate_id = $request->input('associate_id');
			echo $aws_access_id = $request->input('aws_access_id');
			echo $aws_secret_key = $request->input('aws_secret_key');
			$category = 'iphone';$page = 1;
			$country = $request->input('country');
			if(array_key_exists($country, $countryMapping)){
				$country = $countryMapping[$country];
