...
private function get_html_content($url,$token) {
		Log::info("start ".$url."  ".$token);      ///admin/pages.json  
	/*	$ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://www.bbqing.com/");
        ...
        Log::info("data is   ".$response);
         $res = preg_match_all('/window.ISAAC(.*)<\/head>/sm',$response,$matches);
            if($res){
                Log::info("infoshore isaac.js script found".json_encode($matches) );
                $dataaac = $matches[1][0];
                 if(strpos($dataaac, '<ISAAC') !== false){
                 	return 1;
			    }
			}// res if ends
			else {
       ...
       $theme_id = $this->getActiveThemeID($token, $url);//?asset[key]=templates/index.liquid&theme_id=828155753
       ...
       $url = "https://".$url."/admin/themes/".$theme_id."/assets.json?asset[key]=layout/theme.liquid&theme_id=".$theme_id;
		//$data = array("asset" => array("key" => $key, "value" => $value));
		Log::info("url is".$url);
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, $url);
