...
class ProductController extends Controller
{
	...
	public function index()
	{
		...
		$currUser = Auth::User();
		return $currUser->products()->where('status', 'Imported')->with('variants')->with('variantsCount')->with('variants.reviews')->with('variants.mainImage')->orderBy('product_id', 'DESC')->paginate($per_page);
	}
	...
	public function search(Request $request)
	{
		...
		if ($request['query']) {
			$products = $currUser->products()->where('status', 'Imported')->where('title', 'like', '%'.$request['query'].'%')->with('variants')->with('variantsCount')->with('variants.mainImage')->orderBy('product_id', 'DESC');
	...
	public function update(Request $request)
	{
		...
		Log::info("heeyyeyyeyeyyeyeyyeyyeyeyeyeyyey");
	...
	public function addProductByCrawl(Request $request)
	{
		...
		if ($res) {
			...
			if ($res) {
				// Log::info("test flow 2");
				$permission = ProductVariant::where("user_id", $currUser->id)->where("asin", $matches[1][0])->get();
				if (sizeof($permission) > 0) {
					Log::info("test flow 3");
					$failed_productimports = new Failed_productimports(array("user_id" => Auth::user()->id, "url" => $request->input('producturl'), "reason" => 'Product Already Exists', "type" => 'Validation'));
					$failed_productimports->save();
					return response()->json(['error' => ["msg" => ["Product Already Exists"]], ['purl' => $producturl]], 406);
				} else {
					// Log::info("test flow 4");
					// Log::info($permission);
				}
			}
		...
		if ($data != null) {
			...
			if (isset($resObj['Title'])) {
				...
				if (!empty($results['asin'])) {
					$asin = $results['asin'];
				} else {
					$tmpurl = strtok($results['url'], '?');
					$res = preg_match_all("/dp\/(.*)\/ref/U", $tmpurl . "/ref", $matches);
					
					if ($res) {
						// Log::info("278i9888888888888888");
						// Log::info($matches[1][0]);
						$asin = $matches[1][0];
					} 
				}
				...
				if (isset($results['price'])) {
					$price = $results['price'];
					$pattern = '/\$([\d.]+)/'; // This pattern matches a dollar sign followed by digits and optionally a dot
                preg_match($pattern, $price, $matches);
    
          // $matches[1] will contain the extracted price
          if (isset($matches[1])) {
			$price = $matches[1];
        //echo "The price is:$newprice";
         }
        ...
        //echo "The price is:$newprice";
				...
				if (isset($results['list_price'])) {
					...
					$list_price = $this->getAmountnew($list_price);

					//Log::info("price  ".$list_price);
				} else {
					$list_price = $price;
				}
	...
	public function forceSync($id)
	{
		...
		if (sizeof($product) > 0) {
			...
			Log::info("starting surrent user1");
			$domain = parse_url($producturl, PHP_URL_HOST);
			$time_start = $this->microtime_float();
			Log::info($producturl);
			$data = null;
			if ($data == null && $user_id != 17012 && $user_id != 20892) {
			    $data = proxycrawlapi($producturl);
				Log::info("starting surrent user2");
				Log::info($data );
    		}
    		if ($data == null) {

				Log::info("get_html_scraper_api_content3");
    			$data = get_html_scraper_api_content($producturl, $user_id);
				Log::info($data );
    		}
			...
			if ($data == null) {
				...
				if (isset($resObj['Title'])) {
					...
					if ($price > 0) {
						...
						$url = "https://" . $shopurl . "/admin/api/2022-07/variants/" . $shopifyvariantid . ".json";
						$curl = curl_init();
...
{       
    ...
    // Set the URL using the submitted text value
    ...
    return response()->json(['response' => $bodyData]);
}
