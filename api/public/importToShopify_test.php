<?php
 header('Access-Control-Allow-Origin: *');
   ...
   $conn = new mysqli('localhost', 'infoshoreapps_aac', '%@A{}QL;}RE&', 'infoshoreapps_aac');
	...
	$new_products = $conn->query("SELECT * FROM `products` WHERE shopifyproductid = '' and duplicate = 0 and block = 0 and user_id = 17665 and product_id in (SELECT product_id FROM `product_variants` WHERE user_id = 17665 and price > 0)");
