...
if($dataArr['price'] == ''){
    ...
    function proxycrawlapi($producturl) {
        Log::info('Function proxycrawlapi called with URL: ' . $producturl);
    
        $country = getCountry($producturl);
        Log::info('Country determined: ' . $country);
        ...
        if ($country) {
            $url = 'https://api.proxycrawl.com/?token=A8zfXIDXwsj2o5A_1upnJg&autoparse=true&country=' . $country . '&url=' . urlencode($producturl);
            Log::info('URL with country: ' . $url);
        } else {
            Log::info('URL without country: ' . $url);
        }
        ...
        Log::info('Decoded API Response: ' . print_r($res1, true));
    
        if ($res1 && isset($res1['original_status']) && $res1['original_status'] == 200 && isset($res1['body'])) {
            Log::info('Valid API response received.');
    
            $res = $res1['body'];
    
            if (isset($res['name'])) {
                $response['Title'] = $res['name'];
                Log::info('Title: ' . $res['name']);
            }
            if (isset($res['description'])) {
                $response['description'] = $res['description'];
                Log::info('Description: ' . $res['description']);
            }
            if (isset($res['brand'])) {
                $response['brand'] = $res['brand'];
                Log::info('Brand: ' . $res['brand']);
            }
            if (isset($res['breadCrumbs'][0])) {
                $response['category'] = $res['breadCrumbs'][0]['name'];
                Log::info('Category: ' . $res['breadCrumbs'][0]['name']);
            }
            $response['url'] = $producturl;
            $response['currency'] = '';
            if (isset($res['price'])) {
                if (strpos($res['price'], '-') !== false) {
                    $price_arr = explode("-", $res['price']);
                    $response['price'] = trim($price_arr[0]);
                } else {
                    $response['price'] = $res['price'];
                }
                Log::info('Price: ' . $response['price']);
            }
            if (isset($res['inStock']) && $res['inStock'] == true) {
                $response['in_stock___out_of_stock'] = 'In stock.';
                Log::info('Stock status: In stock');
            }
            if (isset($res['images'])) {
                $response['high_resolution_image_urls'] = $res['mainImage'] . '|' . implode("|", $res['images']);
                Log::info('Images: ' . $response['high_resolution_image_urls']);
            }
            if (isset($res['features'])) {
                $response['bullet_points'] = $res['features'];
                Log::info('Features: ' . print_r($res['features'], true));
            }
            ...
            if (isset($response['high_resolution_image_urls'])) {
                ...
                Log::info('Processed images: ' . print_r($images, true));
            }
            if (isset($res['productInformation'][0])) {
                for ($i = 0; $i < count($res['productInformation']); $i++) {
                    $k = preg_replace("/[^a-zA-Z0-9\s]/", "", $res['productInformation'][$i]['name']);
                ...
                $response['productInformation'] = $res['productInformation'];
                Log::info('Product information: ' . print_r($res['productInformation'], true));
            }
    
            if (isset($res['reviews'])) {
                $response['reviews'] = $res['reviews'];
                Log::info('Reviews: ' . print_r($res['reviews'], true));
            }
            ...
            if (isset($res['asinVariationValues'])) {
                ...
                $response['option_name'] = $uniqueVariationNames;
                $response['asinVariationValues'] = $variationValues;
                Log::info('Variation names: ' . print_r($uniqueVariationNames, true));
            }
    
            $output['message'] = json_encode($response);
            Log::info('Final response: ' . $output['message']);
            return $output;
        } else {
            // Log the error details
            Log::error('API call failed or invalid response received.');
            Log::error('Response Status: ' . (isset($res1['original_status']) ? $res1['original_status'] : 'Not set'));
            Log::error('Response Body: ' . (isset($res1['body']) ? json_encode($res1['body']) : 'Not set'));
            return null;
        }
    }
