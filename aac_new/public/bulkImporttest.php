<?php
    ...
    $result = $conn->query("SELECT * FROM `fetchReviews` WHERE `user_id`=17789 and status = 0");
    if($result->num_rows > 0){
        while($row = $result->fetch_assoc()){	  
            $asin = trim($row['product_asin']);
            $conn->query("update fetchReviews set product_asin = '".$asin."' where user_id = 17789 and id = ".$row['id']);
        }
    }
    exit;
   /* $result = $conn->query("SELECT * FROM `product_variants` WHERE user_id = 17789 and asin not in (select product_asin from fetchReviews where user_id = 17789) and asin != '' and shopifyproductid != ''");
    if($result->num_rows > 0){
        while($row = $result->fetch_assoc()){	  
            $asin = $row['asin'];
            $conn->query("INSERT INTO fetchReviews(user_id, product_asin, status, created_at, updated_at) VALUES ('17789', '".$asin."', 0, now(), now())");
        }
    }
    exit;*/
	...
	$restop = $conn->query("SELECT * FROM `bulk_imports` where id = 82");
	    ...
	    if($userObject = $conn->query("SELECT * FROM `users` WHERE `id`='".$data['user_id']."'")){
	        while($userRow = $userObject->fetch_assoc()){	            
	            ...
	            if($skuconsumed < $skulimit ){
                    $asins = explode("\n", $data['asin']);
					$existingASINs = getExistingASINs($data['user_id']);
					$existingASINs = array_map(function($piece){
						return (string) $piece;
					}, $existingASINs);
					$pendingASINs = array();
					foreach($asins as $asin){	
						$asin1 = trim($asin);
						if(!in_array($asin1, $existingASINs)){
							$pendingASINs[] = trim($asin);
						}
					}
					$pendingASINs1 = array_slice($pendingASINs, 0, 5);
                    addlog("asins being performed now".json_encode($pendingASINs), "INFO");
                    $pendingSKUCnt = $skulimit - $skuconsumed;
                    addlog("pending asins count are now".$pendingSKUCnt,"INFO");
				    $skuconsumed = $skuconsumed + addMultipleProduct($pendingASINs1, $data['amazon_base_url'], $pendingSKUCnt, $userRow, $data['id']);
		...
		foreach($asins as $asin) {
			if($asin != "") {
				if($pendingSKUCnt > 0 && $asin != "") {
					$temp = fetchProductDataWithRetry($base_url."/dp/".$asin."/?th=1&psc=1");
					if($temp !== "ERROR") {
...
//https://www.amazon.com/gp/product/B07C5992QM
	...
	function getExistingASINs($user_id) {
		global $conn;
		$existingASINs = array();
		$result = $conn->query("select distinct asin from product_variants where user_id = ".$user_id);
		if($result->num_rows > 0){
			while($row = $result->fetch_assoc()) {
				$existingASINs[] = trim($row['asin']);
			}
		}
