...
export default function ImportProductByURL() {
  ...
  const previewbutton = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/fetch-amazon-product?lang=en-us`,
      ...
      setResponseData2(response.data.body); // Set response data in state
      console.log('fetch-amazon-data',response.data.body);
      setActive(true);
    } catch (error) {
      console.error(error);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/product/addmultiple?lang=en-us`,
        { producturl },
        {
          headers: {
