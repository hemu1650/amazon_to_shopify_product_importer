...
app.controller('ProductAddCtrlnew', function ($scope, ProductService, $stateParams, SweetAlert , $http, $translatePartialLoader,CategoryService,$rootScope,trans,Notification) {
  ...
  /* $scope.addProduct = function(product) {
	...
	*/
	...
	$scope.addmultiple = function(product) {
		if(product['producturl']){
			
			if(product['producturl'].includes('slredirect')){
				var url_string = product['producturl'];
				var url = new URL(url_string);
				var c = url.searchParams.get("url");
				var uri_dec = decodeURIComponent(c);
		        product['producturl'] = "https://www.amazon.com"+c;
		    }
			...
			if(product['producturl'].includes('amazon')){
				...
				$scope.pageObj = {'producturl':''};
				 ProductService.addmultiple($scope.tmp).then(function(data) {
					for(var s=0;s<$scope.products.length;s++){
					...
					$scope.products = d;*/
					//$scope.pagination = $scope.products.metadata;
					$rootScope.$broadcast('product.addmultiple',data[1][0]['product_id']);
				},function(response){
					if(response['data'][0]){
						for(var s=0;s<$scope.products.length;s++){
